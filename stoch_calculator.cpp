...
double Stoch_calculator::Get_K_stochastic(int timeframe_num, QString symbol) {
  ...
  //request (symbol,time interval,start period,end period)
  ...
  this->last_close_price = data[k_period + smooth - 2][3];
  ...
  double real_K_stochastic = (sum_stochastic / smooth) * 100;


  this->all_K_stochastic.push_back(real_K_stochastic);
  this->num++;




  return real_K_stochastic;
...
double Stoch_calculator::Get_D_stochastic(int period) {
  ...
  if (this->num >= period) {
    ...
    return SUM_D_stochastic / period;
...
double Stoch_calculator::Get_last_close_price() {

  return Stoch_calculator::last_close_price;
